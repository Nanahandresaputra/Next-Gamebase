            ...
            <div className='col-span-6 lg:col-span-5 grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-5'>
                {
                    listDataGames.map((data, index) => (
                        isLoading ? <SkeletonCardGames key={index} /> : <GameCard key={index} imgUrl={data.background_image} title={data.name} rating={data.rating} onClick={() => router.push('/detail')} />
                    ))
                }
                <div className=' col-span-2 lg:col-span-4 flex justify-center w-full mt-14'>
                    {/* <PaginationComp /> */}
                    {!isLoading && <Button type='outlined' shape='circle' onClick={() => {
